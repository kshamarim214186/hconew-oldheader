...
export default async function ProjectPage({ itemObj }) {
   
   const properties = getProjectDetails(itemObj);
   const result = await properties;
   
   const message = result.message;
   const propData = result.prop;
   const galleryData = result.gallery;
   const faqData = result.faq;
   const keyplanData = result.keyplan;
   const amenitiesData = result.amenities;
   const floorcategory = result.floorcat;
   const specategory = result.specificationcat;
   const viewedmost = result.mostviewed;
   const otherprop = result.otherprop;
   const neighcat = result.neighcat;
                     ...
                     {/*done*/}
                           ...
                           {keyplanData.length > 0 && 
                              <ul className={styles.highlights}>
                                 {keyplanData.map(function(keydata, index) {
                                    return (
                                       <li key={keydata.id}>
                                          <FontAwesomeComponent iconKey={keydata.iconid} />
                        ...
                        {floorcategory.length > 0 &&
                           <div className={styles.priceFloors}>
                              ...
                              <Tab.Container id="tabPriceAndFloors" defaultActiveKey={"BHK"+result.floorselected}>
                                 <Fancybox
                                  options={{
                                    Carousel: {
                                      infinite: false,
                        ...
                        {neighcat.length > 0 &&
                           <div className={styles.locality}>
                              ...
                              <Tab.Container id="tabLocality" defaultActiveKey={result.neighelected}>
                                 <Nav variant="tabs">
